...
import type { Customer, CustomerType, CustomerFormData, Person, PersonFormData } from '../types';
...
export function CustomerDetail({ customer, onClose, onBack, onSave, onDelete, isMobile, onCustomerUpdate }: CustomerDetailProps) {
  ...
  useEffect(() => {
    ...
    // Load persons for this customer
  ...
  const loadPersons = async () => {
    try {
      const data = await personApi.getByCustomer(customer.id);
      setPersons(data);
    } catch (error) {
            ...
            <div className="bg-gray-50 rounded-lg p-4 min-h-[100px]">
              {customer.notes ? (
                <div className="prose prose-sm max-w-none">
                  <ReactMarkdown>{customer.notes}</ReactMarkdown>
                </div>
              ) : (
