...
interface MemberListProps {
  ...
  onFilterChange?: (filters: { search?: string; memberTypeId?: number }) => void;
}
...
export function MemberList({
  ...
  const [memberTypeId, setMemberTypeId] = useState<number | "">("");
  const [activeFilters, setActiveFilters] = useState<
    { search?: string; memberTypeId?: number } | null
  >(null);

  const hasActiveFilters = activeFilters?.search || activeFilters?.memberTypeId;
          ...
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            {hasActiveFilters && (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {activeFilters?.memberTypeId && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-full">
                    Art: {getMemberTypeName(activeFilters.memberTypeId)}
                    <button
                      onClick={() => {
                        setMemberTypeId("");
                        applyFilters();
